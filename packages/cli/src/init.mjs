...
export async function runInit({ dir, install }) {
  ...
  if (existsSync(target) && statSync(target).isDirectory()) {
    // create-triscope itself rejects non-empty dirs; here we just warn early
    // if the path looks like an existing project so the user has a chance
    // to bail before the scaffolder errors.
    try {
