...
describe('runList', () => {
  ...
  it('exits 1 when the response is non-ok', async () => {
    fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok: false, status: 500, json: async () => ({}) });
    await expect(runList({})).rejects.toThrow(/exit:1/);
  ...
  it('exits 2 when manifest is null (no lab loaded yet)', async () => {
    fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok: true, status: 200, json: async () => null });
  ...
  it('prints the manifest JSON when present', async () => {
    ...
    fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok: true, status: 200, json: async () => manifest });
    await runList({});
  ...
  it('respects a custom url, stripping trailing slashes', async () => {
    fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok: true, status: 200, json: async () => ({}) });
